...
const authMiddleware = (req, res, next) => {
  ...
  try {
    ...
    req.user = decoded; // Store complete user info
    next();
  } catch (err) {
...
const authorizeDepartment = (allowedDepartments) => {
  return (req, res, next) => {
    if (!req.user || !req.user.department) {
      return res.status(403).json({ success: false, message: 'Access denied. User information missing.' });
    }
    
    if (!allowedDepartments.includes(req.user.department)) {
      return res.status(403).json({ 
        success: false, 
        message: `Access denied. ${req.user.department} department not allowed.` 
      });
    }
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Create token with user information including department
...
app.post("/api/verify-token", authMiddleware, async (req, res) => {
  try {
...
// Department-based redirection endpoint
app.get("/api/get-redirect-path", authMiddleware, async (req, res) => {
  try {
    ...
    // Department-based redirection logic
